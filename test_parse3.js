...
let processedLines = lines.map(line => {
    ...
    if (cleanLine.startsWith('"') && cleanLine.endsWith('"') && cleanLine.length >= 2) {
        ...
        // We might need to unescape "" to " since the Excel export wrapped the whole row in "
        // Wait, if it's ""Hello, world"", we want "Hello, world". 
        // Let's just try replacing "" with " for fields that should be quoted.
        // Actually, if we just split by comma here we can see what it looks like.
    }
    return cleanLine;
});
